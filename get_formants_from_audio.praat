...
clearinfo  # Clear praat output window

inDir$ = chooseDirectory$: "Select folder of .wav files"  # Open folder dialog
inDirWav$ = inDir$ + "/*.wav"  # Search folder for WAVs and create list
wavList = Create Strings as file list: "wavList", inDirWav$
numFiles = Get number of strings
if numFiles == 0  # Exit if no WAV files found
	exitScript: "No .wav files found in selected folder"
endif

form Output Style  # Display form to enter output option
	choice Output_style 2
		button Averages
		button Full
endform

appendInfoLine: "Working directory: ", inDir$  # Print directory

number_of_formants = 5  # Number of formants to analyse for

for fileNum from 1 to numFiles  # Loop through files
	...
	for f to number_of_formants  # Add intensity columns to table
		index = Get column index: "F" + string$(f) + "(Hz)"
		Insert column: index + 1, "I" + string$(f)
	endfor

	for row to nRows  # Loop through rows in table
		select table
		time = Object_'table'[row, "time(s)"]

		for f to number_of_formants  # Loop through formants, add freq
			frequency = Object_'table'[row, "F" + string$(f) + "(Hz)"]

			select spectrogram  # Get intensity from spectrogram
			if frequency != undefined
				intensity = Get power at: time, frequency
			else
				intensity = undefined
			endif
			select table  # Add intensity to table
			Set string value: row, "I" + string$(f), fixed$(intensity, 3)
		endfor

		select pitch  # Get fundamental freq. at each time interval
		pitchValue = Get value at time: time, "Hertz", "Nearest"
		select table  # Add pitch to table
		Set string value: row, "Pitch", fixed$(pitchValue, 3)
	endfor

	select table  # Variables to store no. rows, columns
	nRows = Get number of rows
	...
	for column to nColumns  # Loop through columns
		select table
		total_val = 0.0
		val_count = 1
		column_label$ = Get column label... column
		for row to nRows  # Loop through rows
			current_val = Object_'table'[row, column_label$]  # Get value
			if current_val != undefined
				total_val = total_val + current_val  # Add up values
				val_count = val_count + 1
			endif
		
		endfor

		average_val = total_val / val_count  # Calculate average of values
		Set string value: nRows, column_label$, fixed$(average_val, 3)
		appendInfoLine: column_label$, " Average: ", average_val
		
		select avgTable  # Insert average value to average table
		Append column: column_label$
		Set string value: 1, column_label$, fixed$(average_val, 3)

	endfor

	outFile$ = sound_name$ + ".csv"  # Create output file name
	if output_style$ == "Full"  # If user selected "Full"
		select table  # Output = table
		
	else  # If user selected "Average"
		select avgTable  #  Output = average table
	endif

	Save as comma-separated file: inDir$ + "/" + outFile$  # Write file
